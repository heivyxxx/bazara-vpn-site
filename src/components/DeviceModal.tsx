...
export const DeviceModal: React.FC<DeviceModalProps> = ({ isOpen, onClose, currentDevices, totalLimit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] w-full h-full bg-black/80 flex flex-col justify-end sm:items-center sm:justify-center">
      ...
      <div className="relative w-full max-w-md bg-[#18181b] rounded-t-3xl sm:rounded-3xl flex flex-col animate-page-in shadow-2xl border border-white/5 max-h-[90vh] overflow-hidden">
        
        {/* Header with Close */}
        <div className="flex flex-row items-center justify-between px-6 pt-5 pb-4 bg-[#18181b] relative z-20 shadow-sm border-b border-white/5">
          <h2 className="text-white font-extrabold text-xl">Настройка устройств</h2>
        ...
        <div className="flex flex-col px-6 py-4 overflow-y-auto">
          {activeTab === 'add' ? (
            <div className="flex flex-col gap-5 animate-page-in h-full">
              
              <div className="bg-[#fe6125]/10 border border-[#fe6125]/20 rounded-xl py-3 px-4 flex items-center gap-3">
              ...
              <div className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex flex-col gap-3 mt-1">
                   ...
                   <div className="flex-1 bg-black/40 border border-white/10 rounded-xl h-12 flex items-center justify-between px-4 overflow-hidden focus-within:border-[#fe6125]/50 transition-colors">
                     ...
                     <div className="flex items-center">
                       <input 
                         ...
                         onChange={(e) => setAddAmount(Math.min(remaining, Math.max(0, parseInt(e.target.value) || 0)))}
              ...
              <button 
                className={`py-4 w-full rounded-2xl font-bold text-base transition-all ${addAmount > 0 ? 'bg-gradient-to-r from-[#fe6125] to-[#ff9e5e] text-white shadow-lg shadow-[#fe6125]/30' : 'bg-white/5 text-[#6A6D82] cursor-not-allowed border border-white/5'}`}
            ...
            <div className="flex flex-col gap-5 animate-page-in h-full pb-4">
              ...
              <button className="mt-8 py-4 w-full rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 transition-colors flex items-center justify-center gap-2 group text-white">
